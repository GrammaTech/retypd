...
class RecursiveSchemaTest(unittest.TestCase):
    def test_recursive(self):
        ...
        self.assertEqual(
            F_sketches.lookup(parse_var("F.out")).lower_bound,
            parse_var("#SuccessZ"),
        )
        self.assertEqual(
            F_sketches.lookup(parse_var("F.in_0.load.σ4@4")).upper_bound,
            parse_var("#FileDescriptor"),
        )
    ...
    def test_recursive_through_procedures(self):
        ...
        self.assertIsNotNone(g_sketch.lookup(parse_var("g.in_0.load.σ4@4")))
        self.assertEqual(
    ...
    def test_multiple_label_nodes(self):
        ...
        self.assertIsNotNone(f_sketch.lookup(parse_var("f.in_0.load.σ4@0")))
        # we should have this capability even if we don't know the type
        # of the field
        self.assertIsNotNone(f_sketch.lookup(parse_var("f.in_0.load.σ4@12")))
        self.assertEqual(
            f_sketch.lookup(parse_var("f.in_0.load.σ4@0")),
            f_sketch.lookup(parse_var("f.in_0")),
        )
        self.assertEqual(
            f_sketch.lookup(parse_var("f.in_0.load.σ4@4")),
            f_sketch.lookup(parse_var("f.in_0")),
        )

        # check that information is transferred correctly to "g"
        g_sketch = sketches[DerivedTypeVariable("g")]
        self.assertIsNotNone(g_sketch.lookup(parse_var("g.in_0.load.σ4@0")))
        self.assertIsNotNone(g_sketch.lookup(parse_var("g.in_0.load.σ4@12")))
        self.assertEqual(
            g_sketch.lookup(parse_var("g.in_0.load.σ4@0")),
            g_sketch.lookup(parse_var("g.in_0")),
        )
        self.assertEqual(
            g_sketch.lookup(parse_var("g.in_0.load.σ4@4")),
            g_sketch.lookup(parse_var("g.in_0")),
        )
        # check C types
        gen = CTypeGenerator(sketches, lattice, CLatticeCTypes(), 4, 4)
    ...
    def test_multiple_label_nodes_store(self):
        ...
        # check that information is transferred correctly to "g"
        g_sketch = sketches[DerivedTypeVariable("g")]
        self.assertIsNotNone(g_sketch.lookup(parse_var("g.out.store.σ4@0")))

        self.assertEqual(
            g_sketch.lookup(parse_var("g.out.store.σ4@0")),
            g_sketch.lookup(parse_var("g.out")),
        )
        self.assertEqual(
            g_sketch.lookup(parse_var("g.out.store.σ4@4")),
            g_sketch.lookup(parse_var("g.out")),
        )
        self.assertEqual(
    ...
    def test_regression1(self):
        ...
        self.assertEqual(
            nf_sketches.lookup(parse_var("nf.in_0.load.σ4@4")).upper_bound,
            parse_var("int"),
        )
        self.assertEqual(
            nf_sketches.lookup(parse_var("nf.in_0.load.σ4@0")).upper_bound,
            parse_var("int"),
        )
...
class InferTypesTest(unittest.TestCase):
    def test_input_arg_capability(self):
        ...
        f_sketch = sketches[DerivedTypeVariable("f")]
        self.assertIsNotNone(f_sketch.lookup(parse_var("f.in_0")))
        self.assertIsNotNone(
            f_sketch.lookup(parse_var("f.in_0.load.σ1@0.load.σ4@4"))
        )

        g_sketch = sketches[DerivedTypeVariable("g")]
        self.assertIsNotNone(g_sketch.lookup(parse_var("g.out")))
        self.assertIsNotNone(g_sketch.lookup(parse_var("g.out.load.σ1@0")))
        self.assertIsNotNone(
            g_sketch.lookup(parse_var("g.out.load.σ1@0.load.σ4@4"))
        )
    ...
    def test_input_arg_capability_transitive(self):
        ...
        g_sketch = sketches[DerivedTypeVariable("g")]
        self.assertIsNotNone(g_sketch.lookup(parse_var("g.in_0")))
        self.assertIsNotNone(
            g_sketch.lookup(parse_var("g.in_0.load.σ1@0.load.σ4@4"))
        )
        self.assertIsNotNone(
            g_sketch.lookup(parse_var("g.out.load.σ1@0.load.σ4@4"))
        )
...
class CTypeTest(unittest.TestCase):
    ...
    def test_string_in_struct(self):
        ...
        constraints[strcpy].add(
            parse_cs("strcpy.in_1.load.σ1@0*[nullterm] ⊑ int")
        )
